...
func (p *Publisher) Connect() error {
	...
	// Validate the broker URL up front so a clearly-malformed mqtt_broker fails
	// loudly at startup. A reachable-but-currently-down broker is NOT an error
	// here — ConnectRetry handles it in the background (see below).
	broker := p.cfg.MQTTBroker
	if !strings.Contains(broker, "://") {
		broker = "tcp://" + broker
	}
	if u, err := url.Parse(broker); err != nil || u.Host == "" {
		return fmt.Errorf("invalid mqtt_broker %q", p.cfg.MQTTBroker)
	}

	// Assign the client synchronously (a single write, before any SIA handler
	// goroutine starts, so the unsynchronized p.client reads elsewhere stay
	// safe), then connect WITHOUT blocking. SetConnectRetry(true) means Connect
	// keeps retrying until the broker is reachable; blocking on the token here
	// would gate the SIA listener on MQTT being up at boot. Discovery and state
	// republish are driven by OnConnect once the connection is established.
...
func main() {
	...
	if len(os.Args) >= 2 && strings.TrimSpace(os.Args[1]) != "" {
		...
		case "-v", "--version":
			fmt.Printf("sia2mqtt v%s\n", Version)
			return
		case "-h", "--help":
			fmt.Printf("usage: sia2mqtt [config-path]\ndefault config path: %s\n", DefaultConfigPath)
			return
		default:
	...
	// Connect to MQTT in the background. With ConnectRetry/AutoReconnect the
	// client keeps retrying until the broker is reachable, so we deliberately do
	// NOT block the SIA listener on it: the daemon must still receive and ACK
	// hub events even if the broker is down at startup. State is persisted
	// locally and RepublishCurrent (in OnConnect) pushes the latest values once
	// the broker comes up. Only a malformed broker URL fails fast here.
	if err := pub.Connect(); err != nil {
		log.Fatalf("[%s] mqtt config error: %v\n", time.Now().Format("15:04:05"), err)
	}
	log.Printf("[%s] MQTT connecting in background to %s\n",
		time.Now().Format("15:04:05"), sanitizeBrokerURL(cfg.MQTTBroker))
